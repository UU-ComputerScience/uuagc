...
import qualified DefRules           as Pass2 (sem_Grammar, wrap_Grammar, Syn_Grammar(..),Inh_Grammar(..))
import qualified GenerateCode       as Pass3 (sem_Grammar, wrap_Grammar, Syn_Grammar(..),Inh_Grammar(..))
...
compile flags input output
 = do (output0,parseErrors) <- parseAG (searchPath flags) (inputFile input)
      ...
      let output1 = Pass1.wrap_AG      (Pass1.sem_AG                                output0 ) Pass1.Inh_AG      {Pass1.options_Inh_AG      = flags}
          ...
          output4 = PrCod.wrap_Program (PrCod.sem_Program (Pass3.output_Syn_Grammar output3)) PrCod.Inh_Program {PrCod.options_Inh_Program = flags}
      ...
      if not (PrErr.isWarning_Syn_Errors output5) 
       then exitFailure
                   ...
                   blocks1                    = Map.unionWith (++)  (Pass1.blocks_Syn_AG output1) (Pass3.blocks_Syn_Grammar output3)   
