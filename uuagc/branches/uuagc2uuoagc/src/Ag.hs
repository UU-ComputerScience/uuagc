...
import qualified DumpExecutionPlan    as DumpExec (sem_ExecutionPlan, wrap_ExecutionPlan, Syn_ExecutionPlan(..), Inh_ExecutionPlan(..))
...
compile flags input output
 = do (output0,parseErrors) <- parseAG flags (searchPath flags) input
      ...
      let output1   = Pass1.wrap_AG              (Pass1.sem_AG                                 output0 ) Pass1.Inh_AG       {Pass1.options_Inh_AG       = flags}
          ...
          dumpexec  = DumpExec.wrap_ExecutionPlan  (DumpExec.sem_ExecutionPlan grammar3a) DumpExec.Inh_ExecutionPlan {DumpExec.pragmaBlocks_Inh_ExecutionPlan = pragmaBlocksTxt, DumpExec.importBlocks_Inh_ExecutionPlan = importBlocksTxt, DumpExec.textBlocks_Inh_ExecutionPlan = textBlocksDoc, DumpExec.moduleHeader_Inh_ExecutionPlan = mkModuleHeader $ Pass1.moduleDecl_Syn_AG output1, DumpExec.mainName_Inh_ExecutionPlan = mkMainName mainName $ Pass1.moduleDecl_Syn_AG output1, DumpExec.mainFile_Inh_ExecutionPlan = mainFile, DumpExec.inhmap_Inh_ExecutionPlan = Pass3a.inhmap_Syn_Grammar output3a, DumpExec.synmap_Inh_ExecutionPlan = Pass3a.synmap_Syn_Grammar output3a }
      ...
      if not (null errorsToStopOn)  -- note: this may already run quite a part of the compilation...
       then failWith 1
        ...
        do
           ...
           if dumpExec flags'
             then print (DumpExec.result_Syn_ExecutionPlan dumpexec)
             else return ()
