 ...
 = do (output0,parseErrors) <- parseAG (searchPath flags) (inputFile input)
      ...
      if not (PrErr.isWarning_Syn_Errors output5) 
       then exitFailure
                   ...
                   --SM blocks1                    = Map.unionWith (++)  (Pass1.blocks_Syn_AG output1) (Pass3.blocks_Syn_Grammar output3)   
                   --SM (pragmaBlocks, blocks2)    = Map.partitionWithKey (\k _->k=="optpragmas") blocks1
                   --SM (importBlocks, textBlocks) = Map.partitionWithKey (\k _->k=="imports"   ) blocks2
                                      
               --SM writeFile  outputfile . unlines . concat . Map.elems $ pragmaBlocks
               ...
               --SM appendFile outputfile . unlines . concat . Map.elems $ importBlocks
               --SM appendFile outputfile . unlines . concat . Map.elems $ textBlocks
