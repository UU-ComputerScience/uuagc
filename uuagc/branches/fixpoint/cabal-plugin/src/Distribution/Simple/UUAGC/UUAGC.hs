...
uuagcUserHook' uuagcPath = hooks where
  hooks = simpleUserHooks { hookedPreProcessors = ("ag", ag):("lag",ag):knownSuffixHandlers
                          ...
                          , postConf  = uuagcPostConf  uuagcPath
                          }
  ag = uuagc' uuagcPath
...
uuagcPostConf
  :: FilePath
     ...
     -> LocalBuildInfo
     -> IO ()
uuagcPostConf uuagcPath a cf pd lbi = do
  let lib         = library pd
      exes        = executables pd
      bis         = map libBuildInfo (maybeToList lib) ++ map buildInfo exes
      classesPath = buildDir lbi </> agClassesFile
  ...
  fileOptions <- forM options (\ opt ->
      ...
      in do case configVerbosity cf of
  ...
  writeFileOptions uuagcPath fileOptions
